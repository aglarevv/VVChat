...
ChatService::ChatService(){
    ...
    //获取好友信息
    handleMap_.insert(
        {GETINFO, std::bind(&ChatService::getInfo, this,
        std::placeholders::_1,
        std::placeholders::_2,
        std::placeholders::_3)}
    );
    ...
    //     std::placeholders::_1)}
    // );
    wf = WordFilter::sharedInstace();

    wf->load("/home/aglarevv/Chat/chatServer/src/server/other/SensitiveWords.txt");
}
//登录
void ChatService::login(const muduo::net::TcpConnectionPtr& conn, json& js, muduo::Timestamp time){
    ...
    if(user.getPassword() == password && (account != "" && password != "")){
        //登录成功
...
void ChatService::sendFile(const muduo::net::TcpConnectionPtr& conn, json& js, muduo::Timestamp time){
    std::string type = js["msgType"];
    ...
    // // 将文件发送回客户端
    // std::ifstream inputFile("received_file.txt", std::ios::binary);
...
//获取好友信息
void ChatService::getInfo(const muduo::net::TcpConnectionPtr& conn, json& js, muduo::Timestamp time){
    //获取好友信息
    std::string account = js["account"];
    int msgtype = js["msgType"];
    json response;
    if(msgtype != GETINFO){
        response["msgType"] = GETINFO_ACK;
        response["errorNo"] = 1;
        response["errorMsg"] = "获取信息失败";
        conn->send(response.dump());
    }else{
        auto friendVec = friendModel_->query(account);
        if(!friendVec.empty()){
...
void ChatService::regist(const muduo::net::TcpConnectionPtr& conn, json& js, muduo::Timestamp time){
    if(js["name"] == "" || js["password"] == "")
    {
        ...
        if(friendModel_->insert(user.getAccount(), "1")){
            if(state){
                //注册成功
                json response;
                response["msgType"] = REGISTER_ACK;
...
void ChatService::oneChat(const muduo::net::TcpConnectionPtr& conn, json& js, muduo::Timestamp time){
    ...
    if(sendToAccount == "1"){
        ...
        OpenAI oai;
        Conversation convo;
        convo.SetSystemData("You are a helpful bot.");
        convo.AddUserData(msg);
        if (oai.auth.SetKey("sk-ejiCK8hRfa8mwM0V3GHGYWEcNwW6rQYfimNsgfuWdXKo93YM")) { 
            try {
                ...
                json aiJs;
                aiJs["name"] = "AI助手";
                aiJs["msgType"] = ONE_CHAT;
                aiJs["time"] = js["time"];
                aiJs["message"] = convo.GetLastResponse();
                std::cout << aiJs << std::endl;
                conn->send(aiJs.dump());
            }
        ...
        if(user.getState() == "online"){
            ...
            redis_.publish(sendToAccount, js.dump());
            return;
        }
        //不在线
...
void ChatService::addFriend(const muduo::net::TcpConnectionPtr& conn, json& js, muduo::Timestamp time){
    ...
    if(friendModel_->insert(userAccount, friendAccount)){
        responsejs["msgType"] = FRIEND_ADD_ACK;
        ...
        responsejs["errorMsg"] = "添加失败,请检查好友账号是否正确输入";
        conn->send(responsejs.dump());
    }
...
void ChatService::chatGroup(const muduo::net::TcpConnectionPtr& conn, json& js, muduo::Timestamp time){
    ...
    //std::cout << "before msg: " << msg << std::endl;
    wf->censor(msg);
    js["message"] = msg;
    //std::cout << "after msg: " << msg << std::endl;
...
void ChatService::clientCloseException(const muduo::net::TcpConnectionPtr& conn){
    ...
    redis_.unSubscribe(user.getAccount());
    //断开服务器连接
    if(user.getAccount() != "" && user.getAccount() != "1"){
        user.setState("offline");
        userModel_->updataState(user);
    }
}
