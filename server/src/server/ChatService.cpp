...
//获取好友信息
void ChatService::getFriend(const muduo::net::TcpConnectionPtr& conn, json& js, muduo::Timestamp time){
    //获取好友信息
    std::string account = js["account"];
    json response;
...
//获取群组信息
void ChatService::getGroup(const muduo::net::TcpConnectionPtr& conn, json& js, muduo::Timestamp time){
    //获取群组信息
    std::string account = js["account"];
    json response;
